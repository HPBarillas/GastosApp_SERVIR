# GastosApp_SERVIR
 Project for the SERVIR test develop by Hector Pineda
...
*Backend *: Develop by Hector Eduardo Pineda Barillas
...
1. `Cd` to your Laravel app, if there is not a Laravel project can create one with the `composer create-project laravel/laravel:^9.0 GastosApp`
2. Type in your terminal: `composer require laravel/ui` and `php artisan ui vue --auth`
3. Install npm to run the server `npm install`
4. Install this preset via `composer require laravel-frontend-presets/material-dashboard`. No need to register the service provider. Laravel 5.5 & up can auto detect the package.
5. Run `php artisan ui material` command to install the Argon preset. This will install all the necessary assets and also the custom auth views, it will also add the auth route in `routes/web.php`
(NOTE: If you run this command several times, be sure to clean up the duplicate Auth entries in routes/web.php)
6. In your terminal run `composer dump-autoload`
7. Run `php artisan migrate --seed` to create basic users table
(NOTE: If the data do not load on the tables "organizations `php artisan db:seed --class=organizationSeeder`, Countries `php artisan db:seed --class=countriesSeeder`, States `php artisan db:seed --class=stateSeeder`, Cities `php artisan db:seed --class=citySeeder`)
8. Run the server with the `php artisan serve`
(NOTE: If the server thow an error when start, can add the 
    ,"overrides": {
        "autoprefixer": "10.4.5"
    }
...
- [Hector Pineda Developer](https://github.com/HPBarillas)
